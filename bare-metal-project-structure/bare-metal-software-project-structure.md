...
- [Folder Structure](#Folder-Structure)
    ...
    - [platform](#Platform-Library-Folder)
        ...
        - [platform_config_reference](#reference-platform)
            - [drivers](#drivers-reference-config)
            - [linker](#linker-references)
            - [mpfs_hal](#mpfs-hal-config-reference)
...
In the case of PolarFire SoC, the SoC is a specific FPGA design. The FPGA design programs the FPGA fabric and configures the Microcontroller subsystem (MSS). The platform configurations include linker scripts, platform feature enable/disable selections as well as the MSS configurations which itself rely on SoC configuration metadata.
...
You can organize the content of this folders as best fit your preference or keep your application code in another location.

<a name="Boards-Folder"></a>
### boards
The *boards* folder contains one or more subfolders for each development board and SoC-FPGA design combination supported by the software project. Each subfolder contains a *fpga_design* and a *fpga_design_config* folder under it.

![](./images/folder_structure_l6_boards.png)

<a name="fpga-design-config"></a>
#### fpga_design_config
The *fpga_design_config* folder contains header files specifying the configurations of the PolarFire SoC. The content of this folder is auto-generated from configuration meta-data created using the MSS Configurator.

The content of this folder is used by the mpfs_hal to configure PolarFire SoC during system start-up. Among other things it configures:
...
<a name="libero-tcl"></a>
##### libero_tcl
The *libero_tcl* sub-folder contains a copy or links to source controlled TCL script(s) used to re-generate the MSS Configurator and Libero projects from which the programming bitstream for the SoC-FPGA is generated. Keeping track of these scripts allows re-generating the SoC-FPGA programming bit-stream when unsure of which FPGA design your board is programmed with or look at the finer details of SoC-FPGA detail to help with system debug.

<a name="design-description"></a>
##### design_description
The *design_description* sub-folder contains the meta-data generated by the MSS Configurator used to generate the C data structures containing the PolarFire SoC hardware configuration applied by the mpfs_hal at system start-up.

Depending on the project's settings, the content of this folder can be used to regenerate the content of the board's *fpga_design_config* folder each time the project is built.
...
The typical reason for creating a *platform_config* is to customize reference linker scripts provided in the */src/platform/plform_config_reference* folder.
...
#### drivers_config
This *drivers_config* folder contains configuration for the drivers found in */src/plaform/drivers*. This folder is rarely required. When required, its use will be explained in the driver's documentation.

<a name="Linker-Folder"></a>
#### linker
The *linker* folder contains linker script(s) specific to the project. It can be used where no linker script provided as part of the *platform/platform_config_refernce/linker* folder is suitable for the project or where one of these reference linker scripts need to be modified to suit the project.

The use of this folder is only a suggestion. Nothing in the PolarFire SoC forces you to use such a folder. However, it is recommended to keep modified reference linker scripts in a location outside of the *platform* folder.
...
The use of this folder is only a suggestion. Nothing in the PolarFire SoC tools forces you to use such a folder.
...
The content of this folder is not expected to be modified. The content of this folder should be thought as silicon vendor specific software which is updated/released at regular intervals by the silicon vendor. Avoiding modifications of this folder's content will allow easily updating your project with updates and bug fixes from Microchip.
...
##### fpga_ip
The *fpga_ip* folder contains the source code for FPGA soft-IP peripherals.

<a name="off-chip-folder"></a>
##### off-chip
The *off-chip* folder contains the source code for the on-board peripherals such as sensors,
memory devices or the peripherals connected to the board via expansion boards.

<a name="hal-folder"></a>
#### hal
The *hal* folder contains the Hardware Abstraction Layer(HAL) used by FPGA soft-IP peripheral drivers. This HAL is more generic than the MPFS HAL, allowing use of the same soft-IP peripherals and their drivers across different FPGA device families.
...
As indicated by the name, the *platform_config_reference* folder is provided as a reference implementation of the *platform_config*. The *platform_config_reference* configures the *platform* to the default settings which work out of the box. You are advised not to edit the contents of this folder. Instead, refer **platform_config** section above to understand the process of customizing the *platform* for your application.
...
The *soc_config_generator* folder contains the script(s) used to generate the board's configuration file from the design description of the SoC-FPGA design. Currently the design description is provided as a .xml file.
